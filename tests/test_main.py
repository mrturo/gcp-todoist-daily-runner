...
def test_root_endpoint_with_due_datetime_no_tz(monkeypatch):
    ...
    assert any(t["id"] == "5" for t in data["not_overdue_tasks"])
...
def test_root_endpoint_with_no_due(monkeypatch):
    ...
    assert response.status_code == 200
    data = response.json()
    assert any(t["id"] == "4" for t in data["not_overdue_tasks"])
...
def test_root_endpoint_with_invalid_due(monkeypatch):
    ...
    with monkeypatch.context() as m:
        ...
        assert response.status_code == 200
        data = response.json()
        assert any(t["id"] == "3" for t in data["not_overdue_tasks"])
...
def test_root_endpoint_returns_ok(monkeypatch):
    ...
    assert "not_overdue_tasks" in data
    ...
    not_overdue = data["not_overdue_tasks"]
